...
d = ['Je ne vais pas partir', 'pas partir', 'vais pas', 'ne vais pas', 'vais pas partir', 'Je ne vais', 'Je t\'attends', 'Je ne sors pas', 'pas partir d\'ici']
df = pd.DataFrame()
...
Process().score(df_test1, df_ref1['word'], df_ref1['score'], 'Abstract')
...
test5 = Process().nb_pub_by_year(df, 'B', './TestPlot/TestClass_pubYear.svg')
test6 = Process().most_common_bigrams(df, 'D', 3)
test7 = Process().most_common_trigrams(df, 'D', 3)